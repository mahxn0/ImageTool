...
def getname(namepath):
    with codecs.open(namepath,"r","utf-8") as f:
        lines=f.readlines()
    return lines
...
net = load_net("/home/mahxn0/darknet/cfg/yolov3.cfg".encode("utf-8"), "/home/mahxn0/darknet/yolov3.weights".encode("utf-8"), 0)
#meta = load_meta("/home/mahxn0/darknet/cfg/coco.data".encode("utf-8"))
...
class meta:
    classes=81
    names=getname("/home/mahxn0/darknet/data/coco.names")

class yolo_helmet(object):
    def __init__(self):
        pass

    def detect(self, image, thresh=.4, hier_thresh=.5, nms=.45):
...
if __name__ == "__main__":
  ...
  #  picDir = '/home/mahxn0/ROS_workspace/swf'
  #  print("path:",picDir)
  ...
  #          x2=r.getXY(res[4])
  #          y2=r.getXY(res[5])
  #          frame_res=cv2.rectangle(image, (x1,y1), (x2,y2), (0,0,255),4)
  #          cv2.putText(frame_res, res[0] + ' ' + str(res[1]), (x1,y1), font,1,(0,255,0),2)
  #      cv2.imwrite('/home/mahxn0/ROS_workspace/test_result/'+str(i)+'.jpg',frame_res)
  #      i+=1
  #
     ...
     #cap.open("/home/mahxn0/M_DeepLearning/ReId/1.mp4")
#     cap.set(3,1280)
#     cap.set(4,720)
#     cap.set(5,60)
#     cap.set(10,-4)
...
#     cap.set(15,-2)
     ...
     while(cap.isOpened()):
        rect,frame=cap.read()
        #frame=r.rotate(frame,-90)
        frame_res=frame
        if True:
             t1=time.time()
             out = r.detect(frame_res)
             t2=time.time()-t1
             print("detect cost %f ms" % t2)
 #           cv2.imshow("YOLOV3",frame)
             ...
             #左上角和右下角坐标
                if res[0].strip("\n")=='person':
                    ...
                    x2=r.getXY(res[4])
                    y2=r.getXY(res[5])
                    frame_res=cv2.rectangle(frame_res, (x1,y1), (x2,y2), (87,255,123),4)
                    cv2.putText(frame_res, res[0] + ' ' + str(res[1]), (x1,y1), font,1,(0,0,255),2)
                    #frame_rect=frame[y1:y2,x1:x2]
                    #print(frame_rect)
                    #cv2.imwrite('/home/mahxn0/M_DeepLearning/ReId/room/'+str(index)+'.jpg',frame_rect)
                    cv2.waitKey(1)
